...
image: /images/posts/v12/v12-iD-th5.png
date: "2022-04-22"
categories: ['dev']
...
An example of this I wanted to create a website and made a quick research online and found the best way for me was to <CustomLink href="/dev/how-to-start-a-blog/) using [GitHub pages](https://pages.github.com">host my website for free</CustomLink> and use a tool called <CustomLink href="https://jekyllrb.com">Jekyll</CustomLink> to build the site, and a tool called <CustomLink href="https://git-scm.com">Git</CustomLink> to upload my pages online using one command. Both Jekyll and git are command line tools. 

Another example, I want to use <CustomLink href="https://nextjs.org">NextJs</CustomLink> to make webapps, when I go to its <CustomLink href="https://nextjs.org/docs/getting-started">Docs</CustomLink> it will refer me to to install it using the command line.

A third example, I came across a very interesting tool called <CustomLink href="https://www.remotion.dev">remotion</CustomLink>, that allows you to use code to make animations and edit vids. Again, to install it and use it, I had to use the command line.

A fourth example. I came across another tool called <CustomLink href="https://revealjs.com">revealjs</CustomLink>. It allows you to use HTML or Markdown to make presentations that look great, just by typing some text! And I also had to use the cmd line to install it and use it.

### Not just Dev Tools

Package managers are like app stores for the terminal.

<CustomLink href="https://brew.sh">Homebrew</CustomLink> is the most popular package manager for mac computers, and APT for linux, but there are other package managers for both systems.
...
For example, I can install a command-line internet speed test tool on a mac using one command: `brew install speedtest-cli{:bash}`.  And I can test my internet speed through the command line without visiting any website or downloading any app, using a single quick command: `speedtest{:bash}`.

You can also use a package manager to download and update GUI apps using one-line commands. 

I can install Figma from the command line using one line of code: `brew install --cask figma{:bash}`, or install Brave Browser: `brew install --cask brave-browser{:bash}`.
...
## 3. How to start using the command line? 
...
1. <CustomLink href="https://gitforwindows.org/">Install git for windows</CustomLink>, which runs a Linux shell emulator.
2. <CustomLink href="https://gitforwindows.org/">Install Windows Subsystem for Linux</CustomLink>, which is the way that is most recommended. It gives you a Linux environment without using a virtual machine or dual-booting Linux.
...
![blank.png](/images/posts/blank.png)

When you open **the terminal** you get a **prompt**, it might look different depending on your settings. Usually, you'll find a `${:bash}` sign and a fat rectangle. 

The `${:bash}` sign is a **prompt** it shows you that this is **a command line**. You can type your command after the prompt. The rectangle is just **the cursor**.

### `pwd{:bash}` - print working directory

![pwd.gif](/images/posts/pwd.gif)

To know where you're at, type your very first command: `pwd{:bash}`: Print working directory. Think of a directory as just a folder. Basically, you're telling the terminal to show you what folder you are at, right now.

`pwd{:bash}` returns a path for you. Then the prompt and cursor again, show you that you can type another command if you want.

`/Users/mostafalotfy/{:plaintext}`  is the absolute path to where I am currently at, my home folder.

But what is an absolute path?

### Linux Directory Structure And Absolute Paths

![linuxDs.gif](/images/posts/v12/linuxDs.gif)

Everything (all files and folders), in a Unix-based system, is located under the **root** directory `/{:bash}`.
...
In a Linux system the **absolute path** to the **home directory** is like this: `/home/userName{:plaintext}`. 

For mac, it's the same (Since it's a Unix-based system), but your personal files will be under `/Users/yourUserName{:plaintext}`. 

So, the path returned by the `pwd{:bash}` command `/Users/mostafalotfy{:plaintext}`, is the path to my home folder. It's like directions, start from the **root** directory `/{:bash}`, then move to the **Users** directory `/Users{:plaintext}`, then move into my user. 

The home directory has a shortcut to it `~{:bash}`. For example, the absolute path to my **Desktop** will be `/Users/mostafalotfy/Desktop{:plaintext}` or `~/Desktop{:plaintext}`.

If I have an image on my **Desktop** named **screenShot1.png**. The absolute path to it will be `~/Desktop/screenShot1.png{:plaintext}`


### `ls{:bash}` - list directory contents

![ls.gif](/images/posts/v12/ls.gif)

Use the `ls{:bash}` command to know what files and folders are inside your current working directory.

`ls{:bash}` will return a list of all files and folders inside your current directory.

`ls -<options>{:bash}` You can add options to the `ls{:bash}` command.

`ls -a{:bash}` The `-a{:bash}` option will include dotfiles, which are hidden files, in the list.

There are many options for the `ls{:bash}` command. `ls -G{:bash}` will give colored output, which will make directories have a different color than files. 

Most options can be combined. This is how I often use the `ls{:bash}` command: `ls -GhalS{:bash}`

### `clear{:bash}` - clear the terminal screen

![clear commmand](/images/posts/v12/clear.gif)

When the screen gets too crowded you can use the `clear{:bash}` command to get back to the comfort of a blank screen.

### `man{:bash}` - display manual pages

But how to know what options you can use with your commands?

![man command](/images/posts/v12/man.gif)

Use the `man{:bash}` command followed by the command you want to know more about. And you'll get all the info on this command, including a list of all options you can use. 
...
If you run the command `man ls{:bash}`, you can read what each of the options I use with the `ls -GhalS{:bash}` command does.

- `-G{:bash}` gives colored output, so folders have a different color than files. 
-  `-a{:bash}` shows hidden files
- `-l{:bash}` gives a long list with more info about the file. This will come in handy in the future as you expand your knowledge. Including file type, permissions, and file size.
-  `h{:bash}` makes the size of files human-readable, by adding K for Kilobytes, G for Gigabytes, and so on. 
- `S{:bash}` orders files according to size

Try to use the man command on each new command you learn. To quit the manual just press the `q{:bash}` key.

I find it helpful sometimes to open the man in a new tab. By pressing `cmd t{:plaintext}`, then `man <cmd>{:bash}`, and switching using `cmd 1{:plaintext}`  & `cmd 2{:plaintext}` just like you would inside your browser.
...
![cmdAnatomy.png](/images/posts/cmdAnatomy.png)

```bash title="bash"
<cmd> -<option> <argument>▮
```

- `${:bash}` The dollar sign is a prompt to show you that you can write a command here.
- `▮{:bash}` This rectangle is just to show you where the cursor is at.
- `<cmd>{:bash}` The cmd name. Like `ls{:bash}`
- `<options>{:bash}` Options that you can add to the command. Like `-G{:bash}`
- `<arguments>{:bash}` Inputs that some commands need to run. like a path to a file or directory. If the cmd is to list then I can give directories as input to be listed.

`ls -a ~/Desktop{:bash}`, in this command, `ls{:bash}` is the name of the command, `-a{:bash}` is an option that will list hidden files as well, and `~/Desktop{:plaintext}` is the argument, which is the directory that the command will apply to. So the command will list all files and directories inside the desktop directory, including hidden ones.


### Relative path

If you are at the home directory `~{:bash}`. The Documents directory is already inside it, no need to give an absolute path, we can give a relative path instead. I can just do it this way directly: `ls Desktop{:bash}`, This is called a relative path. 

If an absolute path uses the root as the reference, a relative path uses our current working directory as the reference. 

The current working directory is referred to with a `.{:plaintext}`, so you can `ls .{:bash}` but there is no need to write it here as the `ls{:bash}` command already lists the current working directory if given no argument.

The parent directory is referred to using `..{:bash}`. If your current working directory is the Desktop, and you `ls ..{:bash}` this will list your home directory.

So, I can use the command `ls{:bash}` with no arguments and it will list my current working directory. I can use it with a relative path as an argument `ls Documents{:bash}` if I'm at the home directory. And, I can use it with an absolute path argument `ls ~/Downloads{:bash}`. 
...
## 8. What's next? Command Line Resources 
...
[Here's my cheatsheet](/dev/command-line-cheat-sheet/)

There is also a <CustomLink href="https://www.codecademy.com/learn/learn-the-command-line/modules/learn-the-command-line-navigation/cheatsheet">cheatsheet by Codecademy</CustomLink>, if you don't like how I structured my cheatsheet.
...
<a href="https://flaviocopes.com/page/linux-commands-handbook/" target="_blank" rel="noreferrer">
	<img alt="The Linux Commands Handbook, by Flavio" src="/images/posts/flavioBook.jpeg" style={{maxWidth: '20%', height: 'auto'}} />
</a>
...
<a href="https://www.linuxcommand.org/tlcl.php" target="_blank" rel="noreferrer">
	<img alt="The Linux Command Line Book, by William shotts" src="/images/posts/wsbook.png" style={{maxWidth: '20%', height: 'auto'}} />
</a>
...
This class is a part of the <CustomLink href="https://github.com/ossu/computer-science">Open source society Uni</CustomLink>'s path to a free self taught education in computer science. 

### Tune in

I will continue to explore the command line on my website: <CustomLink href="https://mstflotfy.com/">mstflotfy.com</CustomLink>, and YT channel: <CustomLink href="https://www.youtube.com/c/mstflotfyindieDev">mstflotfy - indieDev</CustomLink> . I'll explore commands that used to confuse me, shell scripts, and vim. I will also explore other Dev subjects, as well as Ui and Ux Design. 