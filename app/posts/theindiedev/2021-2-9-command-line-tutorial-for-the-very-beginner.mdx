...
This article is part of <CustomLink href="/dev/step-by-step-guide-start-blog-with-jekyll/">A Step By Step Guide To Start A Blog With Jekyll.</CustomLink>

The commands in this tutorial work on Mac and Linux. If you are using Windows, you will have to look up <CustomLink href="https://duckduckgo.com/?q=how+to+install+bash+on+windows&t=h_&ia=web">how to install bash on Windows.</CustomLink>

There is no need to memorize any command. Understand what each one does and how to use it. Then use a <CustomLink href="/dev/command-line-cheat-sheet/">cheat sheet</CustomLink> when you get stuck. After a while, you will memorize the commands you repeatedly use.
...
Write the command `pwd` just after the `$` sign. 

A new line will appear under your command with its output. It should look something like this:
...
`ls -a` is a command that has an option `-a`.
...
``` 
`cd` is the command, but it needs an argument. It needs a destination to change the working directory to,  `~/Documents` is the argument that the command requires.
...
[Back: The Command Line](/dev/command-line-very-beginner/) | [Next: The command line cheat sheet](/dev/command-line-cheat-sheet/)
